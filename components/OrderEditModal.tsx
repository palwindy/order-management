...
const OrderEditModal: React.FC<Props> = ({ isOpen, onClose, editingOrder, customers, products, onSave, onDelete }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 overflow-y-auto">
      <div className="bg-white rounded-[40px] w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in duration-300 my-auto border border-slate-100">
          ...
          <div className="space-y-6 sm:space-y-8">
            <div>
              ...
              {editingOrder ? (
                ...
                /* 新規登録時のみ選択可能だが、一度選んだら変えない運用を想定し、通常のセレクト */
                ...
                {tempItems.map((item, idx) => (
                  <div key={idx} className="flex gap-2 sm:gap-3 items-end bg-slate-50 p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-100 relative group/row transition-all hover:bg-white hover:shadow-md">
                    <div className="flex-[4] min-w-0">
                      <label className="block text-[8px] sm:text-[9px] font-black text-slate-400 mb-1.5 uppercase tracking-wider">商品名</label>
                      <select 
